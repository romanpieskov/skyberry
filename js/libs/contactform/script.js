...
// =========================================================================  contact_form   второй обработчик
$(function() {
	//Функция проверяет заполнено ли поле с телефоном
	function formValide() {
		var str = $('#contact_form1 input[name=tel]').val();
		str = jQuery.trim(str);
		if(str.length < 5){
			alert ('Введите телефон');
			return false;
	...
	//при нажатии на кнопку button нужной формы запускаем функцию обработки данных
	$('#contact_form1 .button').live('click', function() {
		if (formValide()) {
			//если форма прошла проверку, выводим блок с текстом ожидания
			$('#contact_form1').before('<h3 id="contact_form_info">Оформление заявки. Подождите...</h3>');
			$('#contact_form1').hide();
			//берем путь php обработчика
			...
			$.post(order_url,{
                    ...
                    email: $('#contact_form1 input[name=email]').val(),
...
$(function() {
	...
	$('#write').click(function(){
		...
		$('#popup1').fadeIn();
		//добавляем к окну иконку закрытия
        $('#popup1').append('<a id="popup_close1"></a>');
		//расчитываем высоту и ширину всплывающего окна что бы вывести окно прямо по центру экрана
        ...
        $('#fade1').css({'filter' : 'alpha(opacity=40)'}).fadeIn();
		return false;
	});

	//функция закрытия окна
	$('#popup_close1, #fade1').live('click', function() {
